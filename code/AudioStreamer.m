...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		[self presentAlertWithTitle:NSLocalizedStringFromTable(@"Broadcast disconnected", @"Errors", nil)
							message:NSLocalizedStringFromTable(@"Check your internet connection", @"Errors", nil)];
...
- (void)startInternal
{
	...
	/*	TODO LUCAS ( we only initialize the player one time)
	#if TARGET_OS_IPHONE			
		//
	...
	eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventEndEncountered)
	{
		...
		@synchronized(self)
		{
			...
			{
				// TODO luk : Tirar excepción desde acá 
				// [self failWithErrorCode:AS_AUDIO_DATA_NOT_FOUND];
				NSLog(@"Streamer data: Data not found (notifying)");
				[[NSNotificationCenter defaultCenter] postNotificationName:@"STREAM_NOT_FOUND" object:nil];	
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (!audioFileStream)
		{
			...
			AudioFileTypeID fileTypeHint = [AudioStreamer hintForFileExtension:@"mp3"];
